...
makeTransactionBodyAutoBalance eraInMode systemstart history pparams
    ...
    -- final fee and change output of less than around 4000 ada (2^32-1 lovelace)
    txbody1 <- first TxBodyError $ -- TODO: impossible to fail now
               makeTransactionBody txbodycontent1 {
                 txFee  = TxFeeExplicit explicitTxFees $ Lovelace (2^(32 :: Integer)),
                 txOuts = TxOut changeaddr
                                (lovelaceToTxOutValue $ Lovelace (2^(32 :: Integer)))
                                TxOutDatumHashNone
                        : txOuts txbodycontent
               }
    ...
    mapM_ (\txout -> checkMinUTxOValue txout pparams) $ txOuts txbodycontent1

    -- check if the balance is positive or negative
    -- in one case we can produce change, in the other the inputs are insufficient
    case balance of
      TxOutAdaOnly _ l -> balanceCheck l
      TxOutValue _ v   ->
        case valueToLovelace v of
          Nothing -> Left $ error "TODO: non-ada assets not balanced"
          Just c -> balanceCheck c
     ...
     case era of
       ShelleyBasedEraAlonzo -> do
         case protocolParamUTxOCostPerWord pparams' of
           Just (Lovelace costPerWord) ->
             if txOutValueToLovelace v >= Lovelace (Alonzo.utxoEntrySize (toShelleyTxOut era txout) * costPerWord)
             then Right ()
             else Left . TxBodyErrorAdaBalanceTooSmall $ txOutValueToLovelace v
