...
readItems config@ForwarderConfiguration{..} loQueue =
  Forwarder.TraceForwarder
    ...
    , Forwarder.recvMsgRequest = \blocking (GetTraceObjects n) -> do
        replyList <-
          case blocking of
            TokBlocking -> do
...
getNTraceObjects n loQueue =
  tryReadTBQueue loQueue >>= \case
    Just lo' -> (:) lo' <$> getNTraceObjects (n - 1) loQueue
    Nothing  -> getNTraceObjects 0 loQueue
