...
import           Control.Monad.Class.MonadSTM.Strict (atomically, newEmptyTMVarIO, putTMVar)
...
listenToForwarder
  ...
  -> (NodeInfo -> IO ())
  -> IO ()
listenToForwarder config@AcceptorConfiguration{..} loHandler niHandler = withIOManager $ \iocp ->
...
acceptorActions config@AcceptorConfiguration{..} loHandler niHandler askForNI False =
  ...
  if askForNI
    then
      ...
      Acceptor.SendMsgRequest TokBlocking whatToRequest $ \replyWithTraceObjects -> do
