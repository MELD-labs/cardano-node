...
prop_RemoteSocket n ep' = ioProperty $ do
  ep <- case ep' of
  ...
  void . forkIO $ runTraceForwarder (mkForwarderConfig ep (return nodeInfo))
                                    forwarderQueue
  threadDelay 1000000
  -- Run the acceptor. It will ask 'TraceItem's from the forwarder
  -- and store them in 'acceptorQueue'.
  void . forkIO $ runTraceAcceptor (mkAcceptorConfig ep weAreDone)
